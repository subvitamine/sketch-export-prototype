...
export default function(context) {
  ...
  if (!hasArtboards) {
    UI.message('Il n\'y a pas d\'Artboard dans ce projet !');
    return;
  }
  ...
  if (fileURL) {
    ...
    let today = dayjs().format('YYYYMMDD-HHmm');
    defaultExportPath = `${documentName}_${today}`;
  }

  let rootPath = dialog.showSaveDialogSync(document.sketchObject, {
    defaultPath: defaultExportPath,
    nameFieldLabel: 'Sélectionnez le répartoire d\'export :',
  ...
  if (fs.existsSync(rootPath)) {
    let confirm = (0 === dialog.showMessageBoxSync(document.sketchObject, {
      ...
      title: 'Confirmation',
      message: 'Le réertoire d\'export existe déjà. Etes-vous certain•e de vouloir le remplacer ?\n\n' + rootPath,
  ...
  let jsonPath = `${rootPath}/data.json`;
  fs.writeFileSync(jsonPath, makeJSON(prototypeData));

  //NSWorkspace.sharedWorkspace().openFile(jsonPath);
  UI.message('Le JSON a été exporté avec succès !');
}
...
function makeJSON(prototypeData) {
